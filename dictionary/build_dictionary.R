...
est <- rbind(
  ...
  d(Variable = "income", Source = "PNADC (VD4017)", PT = "Rendimento habitual do trabalho principal, em reais.", EN = "Usual income from the main job, in reais.", Notes = "NA quando não informado; ver unpaid_family e income_missing."),
  d(Variable = "log_income", Source = "Derivada", PT = "log(1 + income), zero quando income é NA.", EN = "log(1 + income), zero when income is NA.", Notes = "Controle na especificação principal."),
  d(Variable = "unpaid_family", Source = "PNADC (VD4009, V4012)", PT = "1 se trabalhador familiar auxiliar sem remuneração (VD4009 == 10 ou V4012 == 7): renda ausente é um zero verdadeiro.", EN = "1 if unpaid family worker (VD4009 == 10 or V4012 == 7): missing income is a true zero.", Notes = "92% dos VD4017 ausentes. Indicador na especificação principal."),
  d(Variable = "income_missing", Source = "PNADC (VD4017)", PT = "1 se a renda não foi informada e o trabalhador não é familiar auxiliar: não resposta genuína.", EN = "1 if income is not reported and the worker is not an unpaid family worker: genuine non-response.", Notes = "Indicador na especificação principal, separado de unpaid_family."),
