...
long[, component := factor(component,
      ...
      labels = c("Total gap", "Within-group", "Composition"))]
...
write_tex(c(
  "\\begin{table}[H]",
  "\\centering",
  "\\caption{Composition versus within-group decomposition of the education gap}",
  "\\label{tab:decomposition}",
  "\\begin{threeparttable}",
  "\\begin{tabular}{lcccc}",
  "\\toprule",
  "& Total gap & Composition & Within-group & Within-group \\\\",
  "Period & $\\Delta^{\\mathrm{raw}}_q$ & component & component & share (\\%) \\\\",
  "\\midrule",
  blk(main, sprintf("Job groups: formality $\\times$ sector $\\times$ occupation (%d job groups)",
                    main$ncell)),
  "\\addlinespace",
  blk(alt, sprintf("Job groups: formality $\\times$ position $\\times$ sector $\\times$ occupation (%d job groups)",
                   alt$ncell)),
  "\\bottomrule",
  "\\end{tabular}",
  ...
  paste0("\\item Notes: The total gap is the survey-weighted difference in the ",
         ...
         "degree. The composition component holds each job group's non-college exit ",
         "rate fixed and varies only the two groups' allocation across job groups; the ",
         "within-group component holds the college allocation fixed and varies only ",
         "the exit rates inside job groups. Period figures are averages of the quarterly ",
         "components weighted by the quarter's share of the total survey weight. ",
         "A job group is a combination of the listed variables that some worker ",
         ...
         "employer and public sector, each formal or informal). The job-group counts ",
         ...
         sprintf("Job groups occupied by only one education group hold %.1f\\%% and %.1f\\%% ",
         ...
         "falls on the within-group component. Brackets report 95\\% percentile ",
...
write_tex(c(
  ...
  "Convention & Composition & Within-group & Composition & Within-group \\\\",
  "\\midrule",
  ...
  "\\end{tabular}",
  "\\begin{tablenotes}[flushleft]\\footnotesize",
  paste0("\\item Notes: Job groups are formality $\\times$ sector $\\times$ ",
         ...
         "and the symmetric convention. The last row keeps only job groups that both ",
         ...
         "dropped job groups carried. The reallocation index is ",
         "$R_g = \\sum_k (s_{gk,\\text{mid}} - s_{gk,\\text{pre}})\\bar m_{k,\\text{pre}}$, ",
         "with $\\bar m_{k,\\text{pre}}$ the pre-pandemic exit rate of job group $k$ ",
         "pooled across groups: it holds rates fixed and lets only allocation ",
         "move, so $R_g > 0$ means group $g$ shifted towards job groups that were ",
